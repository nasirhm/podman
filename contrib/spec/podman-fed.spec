...
%global commit0 eb053df7843d9ec4aba4ee331e04b1d1d5a4ceee
...
Release: 0.20201210002017061532.packit_configutation.225.g9216be200%{?dist}
Summary: Manage Pods, Containers and Container Images
License: ASL 2.0
URL: https://%{name}.io/
Source0: podman-v2.2.0.rc2.tar.gz
...
%prep
%autosetup -Sgit -n podman-v2.2.0.rc2
...
%changelog
* Thu Dec 10 2020 nasirhm <nasirhussainm14@gmail.com> - 2:v2.2.0.rc2-0.20201210002017061532.packit_configutation.225.g9216be200
- Fix storage.conf to define driver in the VM (Daniel J Walsh)
- Bump github.com/containers/storage from 1.24.1 to 1.24.3 (dependabot-preview[bot])
- Properly handle --cap-add all when running with a --user flag (Daniel J Walsh)
- security: honor systempaths=unconfined for ro paths (Giuseppe Scrivano)
- Add system test for shell completion (Paul Holzinger)
- Bump github.com/onsi/gomega from 1.10.3 to 1.10.4 (Daniel J Walsh)
- Honor the --layers flag (Daniel J Walsh)
- pkg/copy: introduce a Copier (Valentin Rothberg)
- Repeat system pruning until there is nothing removed (Daniel J Walsh)
- Bump k8s.io/apimachinery from 0.19.4 to 0.20.0 (dependabot-preview[bot])
- Bump github.com/opencontainers/selinux from 1.6.0 to 1.7.0 (dependabot-preview[bot])
- auto updates: document systemd unit and timer (Valentin Rothberg)
- archive: move stat-header handling into copy package (Valentin Rothberg)
- Fix spelling mistakes (Daniel J Walsh)
- pkg/copy: add parsing API (Valentin Rothberg)
- make podman play use ENVs from image (Christopher J. Ruwe)
- Correct port range logic for port generation (Matthew Heon)
- Make `podman stats` slirp check more robust (Matthew Heon)
- Add systempaths=unconfined option (Urvashi Mohnani)
- Bump github.com/containers/image/v5 from 5.8.1 to 5.9.0 (dependabot-preview[bot])
- Restore json format for fields as well as whole structs (Jhon Honce)
- pass full NetworkMode to ParseNetworkNamespace (Kevin Wiesmueller)
- Fix network ls --filter invalid value flake (Paul Holzinger)
- Implement pod-network-reload (Matthew Heon)
- generate kube on multiple containers (baude)
- Change name of imageVolumes in container config JSON (Matthew Heon)
- Do not error on installing duplicate shutdown handler (Matthew Heon)
- image sign using per user registries.d (Qi Wang)
- container cgroup path (Valentin Rothberg)
- add comment to #8558 regression test (Valentin Rothberg)
- Docker compat API - containers create ignores the name (Milivoje Legenovic)
- Add APIv2 test for containers-prune (Edward Shen)
- container create: do not clear image name (Valentin Rothberg)
- Add saschagrunert and zhangguanzhang to OWNERS (Daniel J Walsh)
- Bump github.com/containers/common from 0.30.0 to 0.31.0 (dependabot-preview[bot])
- update website link for install instructions (Daniel Helfand)
- Jira RUN-1106 System handlers updates (Jhon Honce)
- enable short-name aliasing (Valentin Rothberg)
- Jira RUN-1106 Volumes handlers updates (Jhon Honce)
- Jira RUN-1106 Network handlers updates (Jhon Honce)
- Do not mount sysfs as rootless in more cases (Matthew Heon)
- Add ability to set system wide options for slirp4netns (Ashley Cui)
- Vendor in containers/common v0.30.0 (Ashley Cui)
- Clarify uid range requirements (Rodrigo Lourenço)
- Close image rawSource when each loop ends (Qi Wang)
- Use PasswordCallback instead of Password for ssh (Anders F Björklund)
- More docker compat API fixes (Milivoje Legenovic)
- rewrite podman-cp (Valentin Rothberg)
- e2e: bump pull timeout to 240 seconds (Valentin Rothberg)
- add @Luap99 to OWNERS file (Valentin Rothberg)
- Support Unix timestamps for `podman logs --since` (Chih-Hsuan Yen)
- Fix some nit (zhangguanzhang)
- Jira RUN-1106 Image handlers updates (Jhon Honce)
- Jira RUN-1106 Container handlers updates (Jhon Honce)
- Add containerenv information to /run/.containerenv (Daniel J Walsh)
- Correct which network commands can be run as rootless (Paul Holzinger)
- Drop default log-level from error to warn (Daniel J Walsh)
- podman, exec: move conmon to the correct cgroup (Giuseppe Scrivano)
- Support --network=default as if it was private (Daniel J Walsh)
- Change bindings to stop two API calls for ping (Jhon Honce)
- hack/podman-socat captures the API stream (Jhon Honce)
- BATS: add new load test (Ed Santiago)
- Add mask and unmask option to --security-opt (Urvashi Mohnani)
- Use Libpod tmpdir for pause path (Matthew Heon)
- Fix `podman images...` missing headers in table templates (Jhon Honce)
- add commas between mount options (baude)
- Do not pass name argument to Load API (Qi Wang)
- target is not tag (baude)
- Fix shell completion for ps --filter ancestor (Paul Holzinger)
- Add support for network ids (Paul Holzinger)
- Validate that the bridge option is supported (Anders F Björklund)
- Add integration test for the bridge options (Anders F Björklund)
- Add podman network create option for bridge vlan (Anders F Björklund)
- Add podman network create option for bridge mtu (Anders F Björklund)
- Do not use "true" after "syslog" in exit commands (Matthew Heon)
- Fix typo in tests (Daniel J Walsh)
- Fix potential race condition in testing (Daniel J Walsh)
- compat create should use bindings (baude)
- Add API for communicating with Docker volume plugins (Matthew Heon)
- BATS: add ping test (Ed Santiago)
- Document volume mounts of source directories do NOT get created (Daniel J Walsh)
- Revert the custom cobra vendor (Paul Holzinger)
- Bump version in README to v2.2.0 (Matthew Heon)
- network connect disconnect on non-running containers (baude)
- Bump master to v3.0.0-dev (Matthew Heon)
- Update release notes for v2.2.0 (Matthew Heon)
- Fix extra quotation mark in manpages. (Matthew Heon)
- Fix option names --subuidname and --subgidname (Daniel J Walsh)
- Do not ignore infra command from config files (Matthew Heon)
- Revert "Allow multiple --network flags for podman run/create" (Luap99)
- Add APIv2 tests for kube generate (Edward Shen)
- Document docker transport is the only supported remote transport (Daniel J Walsh)
- podman network label support (Paul Holzinger)
- runtime: set XDG_* env variables if missing (Giuseppe Scrivano)
- Add support for persistent volume claims in kube files (Alban Bedel)
- Prepare support in kube play for other volume types than hostPath (Alban Bedel)
- Remove varlink support from Podman (Daniel J Walsh)
- Fix problems with network remove (Paul Holzinger)
- Switch from pkg/secrets to pkg/subscriptions (Daniel J Walsh)
- Do not validate the volume source path in specgen (Paul Holzinger)
- Add support for --platform (Daniel J Walsh)
- REST API v2 - ping - fix typo in header (Riyad Preukschas)
- REST API v2 - ping - remove newline from response to improve Docker compatibility (Riyad Preukschas)
- squash (jortkoopmans)
- Not use local image create/add manifest (Qi Wang)
- [CI:DOCS] fix misleading save/load usage (Qi Wang)
- [tutorials:mac-win-client] Fix command ensuring sshd is enabled (Lucendio)
- Fix custom mac address with a custom cni network (Paul Holzinger)
- Bump to v2.2.0-dev (Daniel J Walsh)
- Handle ps container created field as a time.Time (Daniel J Walsh)
- test resource cleanup (Matej Vasek)
- more tests (Matej Vasek)
- not forcing unmount (Matej Vasek)
- few more tests (Matej Vasek)
- add test (Matej Vasek)
- add comment (Matej Vasek)
- fix: unmount container without force (Matej Vasek)
- style: wsl (Matej Vasek)
- fix lint (Matej Vasek)
- Implement containers/{id or name}/archive (Matej Vasek)
- Ensure that --net=host/pod/container/none warn with -p (Matthew Heon)
